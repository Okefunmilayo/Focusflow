...
interface Digest {
  ...
  tasksCompleted: number; focusHours: number; topCategory?: string;
  aiSummary: string; improvements: string[]; recommendations: string[];
  createdAt: string;
}
...
function DigestCard({ digest }: { digest: Digest }) {
  ...
  return (
    <div className="card p-4 sm:p-6">
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-sm font-semibold text-slate-700">{fmt(digest.weekStart)} — {fmt(digest.weekEnd)}</span>
          </div>
          <div className="flex flex-wrap gap-3 text-xs text-slate-500 mt-1">
            ...
            {digest.topCategory && <span>Top: <strong className="text-slate-700 capitalize">{digest.topCategory.toLowerCase()}</strong></span>}
          </div>
        </div>
        <button onClick={() => setOpen(o => !o)} aria-expanded={open} aria-label={open ? 'Hide digest' : 'Read digest'}
      ...
      {open && (
        <div className="mt-4 sm:mt-5 space-y-4 sm:space-y-5 border-t border-slate-100 pt-4 sm:pt-5">
          <div>
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-3xl w-full">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 sm:mb-8">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold text-slate-900">Weekly AI Digest</h1>
          <p className="text-slate-500 text-sm mt-0.5">Claude reviews your week and gives personalised recommendations.</p>
        </div>
        <button onClick={() => generateMutation.mutate()} disabled={generateMutation.isPending}
          className="btn-primary flex items-center gap-2 w-full sm:w-auto justify-center">
          {generateMutation.isPending
            ? <><Loader2 className="w-4 h-4 animate-spin" aria-hidden="true" /> Generating…</>
            : <><RefreshCw className="w-4 h-4" aria-hidden="true" /> Generate Now</>}
        </button>
      </div>

      {generateMutation.isPending && (
        <div className="card p-4 sm:p-6 mb-6 flex items-center gap-3 border-violet-200 bg-violet-50" role="status">
          <Loader2 className="w-5 h-5 text-violet-500 animate-spin flex-shrink-0" aria-hidden="true" />
          <div>
      ...
      {isLoading ? (
        <div className="flex justify-center py-16" role="status" aria-label="Loading digests">
          <div className="w-6 h-6 border-2 border-blue-500 border-t-transparent rounded-full animate-spin" />
        </div>
      ) : digests.length === 0 ? (
        <div className="text-center py-12 sm:py-16 card p-6 sm:p-10">
          <Sparkles className="w-12 h-12 text-slate-200 mx-auto mb-4" aria-hidden="true" />
          <p className="text-slate-500 font-medium">No digests yet</p>
          <p className="text-slate-400 text-sm mt-1 mb-6">Click "Generate Now" to get your first AI-powered weekly review.</p>
          <button onClick={() => generateMutation.mutate()} className="btn-primary mx-auto">Generate My First Digest</button>
        </div>
      ) : (
