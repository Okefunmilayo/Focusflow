  ...
  return (
    <div className="card p-4 sm:p-6">
      <div className="flex items-start justify-between">
        ...
        <div className={`w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center ${color}`} aria-hidden="true">
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-6xl w-full">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 mb-6 sm:mb-8">
        <StatCard icon={CheckSquare} label="Done This Week"  value={stats?.completedThisWeek ?? 0}       sub="tasks"        color="bg-blue-500"   />
        ...
        <StatCard icon={Flame}       label="Streak"          value={`${streak}d`} sub={streak > 0 ? 'keep it up!' : 'start today!'} color="bg-orange-500" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
        <div className="lg:col-span-2 card p-4 sm:p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link to="/tasks" className="text-blue-600 text-sm flex items-center gap-1 hover:underline" aria-label="View all tasks">
              View all <ArrowRight className="w-3 h-3" aria-hidden="true" />
            </Link>
          </div>
          {recentTasks.length === 0 ? (
            <div className="text-center py-8 sm:py-10">
              <CheckSquare className="w-10 h-10 text-slate-200 mx-auto mb-3" aria-hidden="true" />
              ...
              <Link to="/tasks">
                <button className="btn-primary mt-4 flex items-center gap-2 mx-auto text-sm py-1.5 px-3">
                  <Plus className="w-4 h-4" aria-hidden="true" /> Add your first task
                </button>
              </Link>
            </div>
          ) : (
            <ul className="space-y-2" aria-label="Upcoming tasks">
              {recentTasks.map((task) => (
                <li key={task.id} className="flex items-center gap-3 p-2.5 sm:p-3 rounded-lg hover:bg-slate-50">
        ...
        <div className="card p-4 sm:p-6">
              ...
              {stats.categoryBreakdown.map(({ category, count }) => {
                ...
                return (
                  <div key={category}>
                    ...
                    <div className="h-2 bg-slate-100 rounded-full overflow-hidden" role="progressbar" aria-valuenow={pct} aria-valuemin={0} aria-valuemax={100} aria-label={`${category} ${pct}%`}>
          ...
          <div className="mt-6 pt-4 border-t border-slate-100 space-y-2">
            <Link to="/ai-goals" className="flex items-center gap-2 text-sm text-blue-600 hover:underline">
              <Plus className="w-4 h-4" aria-hidden="true" /> Break down a goal with AI
            </Link>
            <Link to="/timer" className="flex items-center gap-2 text-sm text-blue-600 hover:underline">
              <Timer className="w-4 h-4" aria-hidden="true" /> Start a focus session
            </Link>
          </div>
        </div>
      </div>
